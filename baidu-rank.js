...
function findRankInPage(target, kw, page) {
  ...
  return searchPage(kw, page).then(function(serp) {
    var hit = false;
    for (var i = 0; i < serp.length; i++) {
      var item = serp[i];
      if (re.test(item)) {
          var v = (page - 1) * 10 + i + 1;
          hit = {word: kw, url: item, rank: (page - 1) * 10 + i};
          var p = "https://www.baidu.com/s?wd="+kw+"&pn="+(page-1)*10;
          open(p,v);
      }
      if (hit) break;
    }
